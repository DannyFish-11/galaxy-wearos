...
Lumiv 手表伴侣应用。通过 AIP v3 协议与 Lumiv 主系统实时同步，支持语音输入、三态指示器和表盘小部件。
...
| Samsung | Lumiv Watch 4/5/6/7 | Wear OS 3.0+ | ✅ |
...
小米手环不能装 Lumiv APP，但可以作为**通知接收端**使用：

1. 在 Lumiv 主系统中配置手环的蓝牙 MAC 地址
2. Lumiv 通过蓝牙将消息推送到手环显示
3. 手环按钮操作可回传到 Lumiv

这种方式走的是小米蓝牙通知协议，不需要在手环上安装任何应用。
配置方式：在 Lumiv 的 `devices.yml` 中添加手环条目，类型选 `mi_band`。
...
```bash
# 克隆本仓库
git clone <repo-url> lumiv-wearos
cd lumiv-wearos
...
1. 手表上打开 Lumiv APP
2. 进入**设置**
3. 填入 Lumiv 服务器的 WebSocket 地址（如 `ws://192.168.1.100:9000`）
4. 填入 Token
5. 点击**连接**
...
- **三态指示器** — 实时同步 Lumiv 的 SILENT/LIMINAL/MANIFEST 状态
- **语音输入** — 按住说话，语音转文字后通过 AIP 发送给 Lumiv
- **智能体列表** — 查看当前活跃的 Lumiv 智能体
- **表盘小部件** — 在 Wear OS 表盘划动中查看 Lumiv 状态
...
│       ├── AndroidManifest.xml
│       ├── java/com/lumiv/wear/
│       │   ├── LumivWearApplication.kt   # 全局状态和 AIP 客户端
│       │   ├── MainActivity.kt            # 主界面
│       │   ├── VoiceActivity.kt           # 语音命令
│       │   ├── data/
│       │   │   └── AIPClient.kt           # WebSocket + 消息协议
│       │   ├── service/
│       │   │   └── LumivWearService.kt   # 前台服务
│       │   ├── tile/
│       │   │   └── LumivTileService.kt   # 表盘小部件
│       │   └── ui/
│       │       ├── screens/               # 四个页面
│       │       └── theme/                 # 暗色主题
│       └── res/values/
