 ...
 *
 * 设计原则：
 * - 轻量级：Receiver 只做最小化工作，实际发送走协程
 * - 可靠性：使用 goAsync() 确保后台完成后再被系统回收
 * - 复用：直接调用 AIPClient.sendCommand，不新造网络层
 */
class ReplyReceiver : BroadcastReceiver() {

    companion object {
        const val ACTION_REPLY = "com.lumiv.wear.ACTION_DECISION_REPLY"
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        // goAsync: 允许 Receiver 在后台完成协程任务后才被系统回收
        val pendingResult = goAsync()
        val scope = CoroutineScope(Dispatchers.IO + SupervisorJob())

        scope.launch {
            try {
                val app = context.applicationContext as LumivWearApplication
                if (!app.isAipClientReady()) {
                    Log.e(TAG, "AIPClient not initialized — cannot send reply")
                    return@launch
                }
