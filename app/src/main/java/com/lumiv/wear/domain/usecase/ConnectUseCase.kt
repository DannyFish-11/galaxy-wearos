 ...
 *
 * @param app LumivWearApplication for accessing encrypted credentials and client
 */
class ConnectUseCase(
    private val app: LumivWearApplication
) {
    companion object {
    ...
    suspend fun executeWithStoredCredentials(): Boolean {
        return try {
            val prefs = app.getSharedPreferences("lumiv_config", android.content.Context.MODE_PRIVATE)
            val url = try {
